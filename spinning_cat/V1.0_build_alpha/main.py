...
class AnimatedGifApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Spinning Pi-based Interactive Nonsensical System(SPINS)")
        ...
        self.round_delay_active = False  # Flag for delaying round start
        ...
        self.score_added = False      # Ensure score is added only once per game

        # Persistent Scoreboard Storage
        self.scores = []  # List to store total times of completed games
        self.scoreboard_frame = None  # To store the scoreboard frame
        ...
        # New: Scoreboard button (top center)
        self.scoreboard_button = tk.Button(
            self.master,
            text="Scoreboard",
            command=self.show_persistent_scoreboard,
            font=("Arial", 13),
            fg="black",
            bg="yellow"
        )
        self.scoreboard_button.place(relx=0.5, rely=0.0, anchor="n", y=10)

        # Scoreboard and Warning labels (hidden by default)
        self.score_label = tk.Label(self.master, text="", font=("Arial", 24, "bold"), bg=GREEN, fg="white")
    ...
    def toggle_game_mode(self):
        ...
        if self.game_mode:
            ...
            self.image_label.place_forget()
            # Hide scoreboard, warning, play-again if visible:
            if self.scoreboard_frame:
            ...
            # Activate round delay with 1800ms delay
            self.round_delay_active = True
            self.master.after(1800, self.delayed_start_new_round)
            self.tease_button.config(state="disabled")
        else:
            # When stopping game mode, hide scoreboard/warning/play-again.
            if self.scoreboard_frame:
                self.scoreboard_frame.destroy()
                self.scoreboard_frame = None
            self.score_label.place_forget()
    ...
    def delayed_start_new_round(self):
        """ Called after a delay (1800ms) to start the round. """
        self.round_delay_active = False
        self.start_new_round()
    
    def reset_game(self):
        # When play again is hit, also destroy any scoreboard frame if present.
        if self.scoreboard_frame:
            self.scoreboard_frame.destroy()
            self.scoreboard_frame = None
        if self.warning_label:
        ...
        # Place the cat labels so they are visible again.
        for i, lbl in enumerate(self.cat_labels):
            rx, ry = self.cat_positions[i]
            lbl.place(relx=rx, rely=ry, anchor="center")
        # Activate delay before starting new round (1800ms)
        self.round_delay_active = True
        self.master.after(1800, self.delayed_start_new_round)
        self.tease_button.config(state="disabled")
        self.game_button.config(text="Stop Playing")
    ...
    def handle_cat_hit(self, cat_index):
        """ Called when the correct cat is hit. Add a 669ms delay before next round. """
        self.cat_spinning[cat_index] = False
        elapsed = time.time() - self.round_start_time
        self.round_times.append(elapsed)
        self.total_time += elapsed
        self.hits_count += 1
        if self.hits_count < 7:  # 7 rounds
            self.master.after(669, self.start_new_round)
        else:
    ...
    def show_scoreboard(self):
        ...
        if not self.score_added:
            self.scores.append(self.total_time)
            self.score_added = True
        # NEW: Create a scoreboard frame for a styled, multi-line scoreboard.
        self.scoreboard_frame = tk.Frame(self.master, bg=GREEN)
        self.scoreboard_frame.place(relx=0.5, rely=0.5, anchor="center")
        title_label = tk.Label(self.scoreboard_frame, text="The Cats Are Tired Now.", font=("Arial", 30, "bold"), fg="black", bg=GREEN)
        title_label.pack(pady=(0,10))
        for i, t in enumerate(self.round_times):
            round_label = tk.Label(self.scoreboard_frame, text=f"Round {i+1}: {t:.2f} sec", font=("Arial", 21), fg="black", bg=GREEN)
            round_label.pack()
        total_label = tk.Label(self.scoreboard_frame, text=f"Total: {self.total_time:.2f} sec", font=("Arial", 28, "bold"), fg="Blue", bg=GREEN)
        total_label.pack(pady=(10,0))
        self.game_over = True  # Freeze sensor input
        self.play_again_button = tk.Button(
            self.master,
            text="Play Again",
            command=self.reset_game,
    ...
    def show_persistent_scoreboard(self):
        scoreboard_win = tk.Toplevel(self.master)
        scoreboard_win.title("Scoreboard")
        scoreboard_win.geometry("400x400")
        if not self.scores:
            scores_text = "No scores yet.\nPlay with the cats first."
        else:
            scores_text = "\n".join([f"Game {i+1}: {score:.2f} sec" for i, score in enumerate(self.scores)])
        label = tk.Label(scoreboard_win, text=scores_text, font=("Arial", 16, "bold"), fg="black")
        label.pack(expand=True, fill="both")
        close_btn = tk.Button(scoreboard_win, text="Close", command=scoreboard_win.destroy,
                              font=("Arial", 14, "bold"), fg="white", bg="blue")
        close_btn.pack(pady=22)
    
    # ------------------- Polling & Animation -------------------
    def poll_inputs(self):
        if self.game_mode:
            if not self.game_over and not self.round_delay_active:
                ...
                pass  # Skip sensor processing when game is over or delay is active.
        else:
